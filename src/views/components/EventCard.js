...
const EventCard = ({ event, onFavoriteToggle = null }) => {
  ...
  const isFavorite = isBookmarked(event.id);
  ...
  const toTotalSeconds = (cd) =>
    !cd ? 0 : cd.days * 86400 + cd.hours * 3600 + cd.minutes * 60 + cd.seconds;
  ...
  useEffect(() => {
    ...
    const total = toTotalSeconds(parsed);
    const tick = () => {
      const elapsed = Math.floor((Date.now() - start) / 1000);
      setCountdown(formatCountdown(total - elapsed));
    };
    tick();
  ...
  const toggleFavorite = () => {
    toggle(event);
    if (typeof onFavoriteToggle === "function" && isFavorite) onFavoriteToggle(Number(event.id));
  };

  const handleLike = () => {
    const liked = JSON.parse(localStorage.getItem("likes")) || {};
    const newLikes = (liked[event.id] || 0) + 1;
    liked[event.id] = newLikes;
    ...
    >
      <div className="position-relative">
        ...
        />
          ...
          <button type="button" aria-label={isFavorite ? "Remove from favorites" : "Add to favorites"} className="position-absolute top-0 end-0 m-1 p-1 rounded-2 fs-6 fw-bold cursor-pointer"
          style={{
