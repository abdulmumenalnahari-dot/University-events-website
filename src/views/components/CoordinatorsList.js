...
const CoordinatorsList = ({ coordinators = [], defaultTab = 'faculty' }) => {
  ...
  const filteredCoordinators = coordinators.filter(coord => {
    const title = String(coord.title || '').toLowerCase();
    if (activeTab === 'faculty') return !title.includes('student');
    if (activeTab === 'student') return title.includes('student');
    return true;
  });
