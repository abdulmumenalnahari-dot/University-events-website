...
export default function Events(){
  ...
  return (
    <div className="container my-4">
      ...
      <FilterBar
        search={search} setSearch={setSearch} category={category} setCategory={setCategory}  sort={sort} setSort={setSort}  />

      <div className="grid">
        {list.map(e => (<EventCard key={e.id} evt={{
          ...e,
          // لو الصورة نسبية وبدون سلاش، نضمن المسار
          thumbnail: e.thumbnail?.startsWith('/')
            ? `${BASE_URL}${e.thumbnail}`
            : (e.thumbnail || `${BASE_URL}/images/banner1.svg`)
        }} />))}
      </div>
